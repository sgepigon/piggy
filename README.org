...
** Usage
   ...
   We can use ~exercise-args~ to see how the our old and new specs compare.

   #+BEGIN_SRC clojure
     (compat/exercise-args `plus ::new)
     (compat/exercise-args ::old ::new)
     (compat/exercise-args (s/cat :x ::int :y ::int) ::new)
   #+END_SRC

   The three calls to ~exercise-args~ above are equivalent and all return a sequence of ~[val old-conformed-val new-conformed-val]~ tuples, e.g.:
